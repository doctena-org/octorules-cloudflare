...
### Added
- Magic Transit / Layer-4 phases are now linted: packet-field expressions
  (`ip.proto`, `tcp.*`, `udp.*`) validate against the wirefilter Layer-4 scheme
  instead of raising spurious unknown-field and parse errors.

### Changed
- `octorules-wirefilter` is now a required dependency (previously the optional
  `[wirefilter]` extra). The bundled `schemas.json` field snapshot is removed —
  the field and function registry is always built from wirefilter at import.
  `pip install octorules-cloudflare` now installs wirefilter; the `[wirefilter]`
  extra is kept as a no-op for compatibility.

## [0.11.1] - 2026-06-20

### Fixed
- Rules using SDK-aliased `action_parameters` keys — notably
  `set_cache_settings` with `status_code_ttl` — no longer show a perpetual
  no-op `MODIFY` in `plan`/`sync`.
...
### Changed
- Credential error now includes env var hint matching Azure/Google/Bunny pattern

### Fixed
- Test import updated for removed `validate` subcommand in core
...
### Fixed
- ``schemas.json`` regenerated with octorules-wirefilter 0.3.5 (v0.6.0
  shipped with a stale snapshot generated by wirefilter 0.3.3).

### Changed
- Pre-commit hook now runs ``pip install -e ".[dev,wirefilter]"`` before
  checks to ensure dependencies are current.
